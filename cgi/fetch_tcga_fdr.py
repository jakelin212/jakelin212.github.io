    ...
    if (intype == "c_clin" or intype == "b_clin" or intype=="gnab"):
	    ...
	    #for cnvr	
            ...
            if (intype == "gsva" or intype == "drug" ):
                ...
                try:
                    fdrcur.execute("select featurea from [_fdr_pv_lookup]") # where featurea like '" + feature + "%'")
                    row = fdrcur.fetchall()
                    print list(row)
                    print len(row) 
                    """gsvapw = 0.0
            ...
            cuts = [-1, -0.7, -0.3, 0.3, 0.7, 1]
            zscores = vals	
            if (intype == "gexp" or intype == "n_clin" or intype == "mirn" or intype == "meth"):
                zmetrics = zs.zs(vals)
                zscores = zmetrics[2]
                cuts = [-3, -2, -1, 1, 1, 2, 3]
            i = 0
            for z in zscores:
                ...
                if (z < cuts[0]):
                    pa['vlow'].append(xy)
                elif (z > cuts[0] and z < cuts[1]):
                    pa['low'].append(xy)
                elif (z > cuts[4] and z < cuts[5]):
                    pa['high'].append(xy)
                elif (z > cuts[5]):
                    pa['vhigh'].append(xy)
                else:
